...
const BasicInfoComponent: React.FC<BasicInfoComponentProps> = ({ myInfo, edit }) => {
    return (
        <div className="flex flex-col w-full p-14 bg-cuarter rounded-3xl shadow-lg shadow-cuarter transition-all">
          ...
          <div className="w-full flex">
            <div className="flex flex-col">
              ...
              {edit ? <Input className="h-8 mb-3"  value={myInfo.data.birth_date} label={"Fecha de nacimiento: "} /> :<span className="text-lg">Fecha nacimiento: {myInfo.data.birth_date}</span>}
            </div>
            <Image
              width={300}
              height={200}
          ...
          {edit ? <Input className="h-8 mb-3"  value={myInfo.data.age.toString()} label={"Edad: "} /> : <span className="text-lg mb-3">Edad: {myInfo.data.age == 2024 ? "Sin calcular" : "Pendiente"}</span>}
