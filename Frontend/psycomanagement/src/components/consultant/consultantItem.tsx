...
const ConsultantItem = ({ consultant }: ConsultantItemProps): JSX.Element => {
  const [firstName = ""] = consultant.names.split(" ");
  const [lastName = ""] = consultant.last_names.split(" ");
  const initials = `${firstName.charAt(0)}${lastName.charAt(0)}`.toUpperCase();
  const age = (() => {
    const [day, month, year] = consultant.birth_date.split("/");
    if (!day || !month || !year) {
      return null;
    }
    const birthYear = Number(year);
    const currentYear = new Date().getFullYear();
    return Number.isNaN(birthYear) ? null : currentYear - birthYear;
  })();
