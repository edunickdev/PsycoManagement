...
@consultant.get(
    "/{id_therapist}", tags=["Therapist"], dependencies=[Depends(JWTBearer())]
)
...
@consultant.post(
    "/new-consultant", tags=["Therapist"], dependencies=[Depends(JWTBearer())]
)
def create_new_consultant(consultant: Consultant):
    new_consultant = dict(consultant)
    first_condition = get_collection("Consultants").find_one(
        {"email": consultant.email}
    )
    second_condition = get_collection("Consultants").find_one(
...
async def update_consultant(id: str, info: dict):
    ...
    try:
        if (
            sizeFields != 0
            and sizeNewValues != 0
            and sizeFields == sizeNewValues
            and myAnnotation
        ):
            update_fields: dict = {
                field: info["newValues"][i] for i, field in enumerate(info["fields"])
            }
            ...
            annotation = (
                get_collection("Annotations").insert_one(new_annotation).inserted_id
            )
